...
\begin{quadro}[!h] 
\begin{verbatim}
defender: naoDominada(bola) & maisPerto(bola) & not com(bola)
	<- ?posBola(X,Y);
	irLinhaReta(X,Y);
	!defender.
...
O primeiro plano de ação leva em consideração três percepções. Se o agente não dominou, está próximo e não está de posse da bola, irá em direção a ela numa tentativa de defesa.

O segundo plano define que se o agente está com a bola, no caso de uma defesa bem sucedida, por exemplo, deve chutar a bola para longe.

O terceiro plano define que se a bola está longe do agente, deve-se avançar uma pequena distância do gol de forma a aproximar-se dela.

O quarto e último plano define a ação principal de defesa, que consiste em obter a posição da bola, olhar para ela e executar a manobra de defesa.
